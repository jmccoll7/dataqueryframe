...
def test_select_distinct_single_string():
    ...
    expected = SqlDataFrame({'col1': [1, 2], 'col2': [3, 4]})
...
def test_select_count():
    ...
    expected = SqlDataFrame({'col1': [1, 2], 'count': [2, 1]})
    pd.testing.assert_frame_equal(result, expected)
...
def test_where_like():
    ...
    expected = SqlDataFrame({'col1': ['apple', 'banana']}, index=[0, 1])
    pd.testing.assert_frame_equal(result, expected)
...
def test_where_equals():
    ...
    expected = SqlDataFrame({'col1': [2], 'col2': ['b']}, index=[1])
    pd.testing.assert_frame_equal(result, expected)
...
def test_where_not_equals():
    ...
    df = SqlDataFrame(data)
    result = df.where('col1', '!=', 2)
    expected = SqlDataFrame({'col1': [1, 3], 'col2': ['a', 'c']}, index=[0, 2])
    pd.testing.assert_frame_equal(result, expected)
...
def test_order_by():
    ...
    expected = SqlDataFrame({'col1': [1, 2, 3], 'col2': ['a', 'b', 'c']}, index=[2, 0, 1])
    pd.testing.assert_frame_equal(result, expected)
...
def test_order_by_descending():
    ...
    expected = SqlDataFrame({'col1': [3, 2, 1], 'col2': ['c', 'b', 'a']}, index=[1, 0, 2])
    pd.testing.assert_frame_equal(result, expected)

def test_union():
    data_a = {'col1': [1, 2], 'col2': ['a', 'b']}
    data_b = {'col1': [2, 3], 'col2': ['b', 'c']}
    df_a = SqlDataFrame(data_a)
    df_b = SqlDataFrame(data_b)
    result = df_a.union(df_b)
    expected_data = {'col1': [1, 2, 3], 'col2': ['a', 'b', 'c']}
    expected = SqlDataFrame(expected_data).reset_index(drop=True)
    pd.testing.assert_frame_equal(result.reset_index(drop=True), expected)
...
def test_union_non_dataframe():
    ...
    with pytest.raises(ValueError) as e:
        df.union([1, 2, 3])
    assert "The 'other' parameter must be a pandas DataFrame or an instance of SqlDataFrame." in str(e.value)
